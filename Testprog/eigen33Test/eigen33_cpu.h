...
cardano(const T A[3][3], T w[3])
{
  ...
  //       | f*  e*  c  |
    ...
    const T dd = SQR(A[0][1]);				// d^2
    const T ee = SQR(A[1][2]);				// e^2
    const T ff = SQR(A[0][2]);				// f^2
    const T m  = A[0][0] + A[1][1] + A[2][2];
    const T c1 = (A[0][0]*A[1][1] + A[0][0]*A[2][2] + A[1][1]*A[2][2])
	- (dd + ee + ff);
    const T c0 = A[2][2]*dd + A[0][0]*ee + A[1][1]*ff - A[0][0]*A[1][1]*A[2][2]
	- 2.0 * A[0][2]*de;

    const T	p = SQR(m) - 3.0*c1;
    const T	q = m*(p - (3.0/2.0)*c1) - (27.0/2.0)*c0;
    const T	sqrt_p = std::sqrt(std::abs(p));

    T	phi = 27.0 * ( 0.25*SQR(c1)*(p - c1) + c0*(q + 27.0/4.0*c0));
